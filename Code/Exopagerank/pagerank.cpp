...
void calculatePageRankPar()
{
  ...
  procRank, numProcs;
  std::vector<double> xold(N);
  for (int i = procRank*N/numProcs; i < (procRank+1)*N/numProcs; i++) { 
    xold[i] = 1.0 / N; 
  }
  MPI_Allgather(&xold[procRank*N/numProcs],N/numProcs,MPI_DOUBLE,&xold,N,MPI_DOUBLE,MPI_COMM_WORLD);
  ...
  // A FAIRE ...
  std::vector<int> rowCount(N);
  for (int j = procRank*N/numProcs; j < (procRank+1)*N/numProcs; j++) {
    for (int rowPtr = colBegin[j]; rowPtr < colBegin[j + 1]; rowPtr++) { rowCount[rowIdx[rowPtr]]++; }
  }
